select lunch_status_1011
      ,count(base_studentid) as count
from
      (select base_studentid
            ,base_schoolid
            ,base_grade_level
            ,lower(ps_customfields.getStudentscf(id, 'Lunch_Status_1011')) as lunch_status_1011
      from
            (select base_studentid
                   ,base_schoolid
                   ,base_grade_level
            from
                (select re.studentid as base_studentid
                 ...
                 where re.entrydate >= '01-AUG-10' and re.exitdate < '01-JUL-11' and (re.exitdate - re.entrydate) > 0 and re.grade_level = 8)
            where rn = 1
            union all
      ...
      join students s on base_studentid = s.id)
group by lunch_status_1011;